...
# Create an object called pheno_data. It is a dataframe. It has 2 columns called "ids" and "stage".
```{r}
...
# sep = separate
```{r message=FALSE}
#store paths as a list in object. points to our rna seqs data. 
#ballgown is the name of directory everything is found in, so we are adding our sample ID's to it through pheno_data to make a complete path separated by /'s.
samples.c <- paste('ballgown', pheno_data$ids, sep = '/')
#tell ballgown we want to analyze data. data is stored in samples.c and meas = measurements. pheno_data tells computer about characteristics of our data.
bg <- ballgown(samples = samples.c, meas='all', pData = pheno_data)
bg
```

# subset = filter the data. Remove data that has a variance less than 1. Removes low-quality data.
# texpr() will extract transcript-level expression measurements
# rowVars() is a variance estimate for each column in our matrix bg.
```{r}
bg_filt = subset(bg,"rowVars(texpr(bg)) >1",genomesubset=TRUE)
bg_filt
```

# create a table of transcripts. Includes names of genes
```{r}
...
# Shows you Gene name, transcript name, feature, ID, fc, p value and q value.
```{r}
results_transcripts[results_transcripts$transcriptNames == "gene-PA0001", ]
```

# This is filtering for high-quality data. Selects for those with a p value over 0.05 
```{r}
sigdiff <- results_transcripts %>% filter(pval<0.05)
dim(sigdiff)
```

# organize the table by ascending p value. (lowest/best p value is at the top) And higher fold changes at the top?
```{r}
...
# load all of the gene names
```{r}
bg_table = texpr(bg_filt, 'all')
bg_gene_names = unique(bg_table[, 9:10])
```

# pull out gene expression data and store as gene_expression and visualize
```{r}
gene_expression = as.data.frame(gexpr(bg_filt))
head(gene_expression)
```

# Change the names of the columns using c(). 
```{r}
...
## The bar chart generated shows that there are many genes with one transcript, meaning that the gene is downregulated. The most upregulated genes have 75 transcripts for one gene.
```{r}
...
# In order to create a plot of other set, you need to choose a different column name in stead of plank01 or plank02.
# This plot was generated from gene_expression table
```{r}
...
## It means that they have great overlap in what genes they express and in what amounts.

# create plot of differential gene expression between the conditions
# This plot shows the frequency of genes that are expressed when you compare planktonic and biofilm expression.
```{r}
...
# choose a gene to determine individual expression (5 in the bg_filt object)
```{r}
ballgown::transcriptNames(bg_filt)[5]
ballgown::geneNames(bg_filt)[5]
```
# transform to log2
```{r}
...
# This graph shows that for the gene lptA, the planktonic community expresses it a lot more than the biofilm.
```{r}
